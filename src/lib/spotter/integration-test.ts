...
console.log(
  `   Position: E${spotterData.spotterPosition.east} N${spotterData.spotterPosition.north}`
);
...
console.log(
  `   East: ${correctedTarget.east.toFixed(2)} (Δ ${(correctedTarget.east - target.east).toFixed(2)})`
);
console.log(
  `   North: ${correctedTarget.north.toFixed(2)} (Δ ${(correctedTarget.north - target.north).toFixed(2)})`
);
console.log(`   Height: ${correctedTarget.height.toFixed(1)}m\n`);

// Erfolgstest
if (
  correctedTarget.east !== target.east ||
  correctedTarget.north !== target.north
) {
