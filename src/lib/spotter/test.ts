...
export function testTargetCalculation(): void {
  ...
  assertClose(
    target45.east - spotterPos.east,
    target45.north - spotterPos.north,
    0.01
  );
...
export function testLateralCorrection(): void {
  ...
  assertCoordinateClose(correctedRight90, {
    east: 500,
    north: 398,
    height: 50,
  });
...
export function testRoundtrip(): void {
  ...
  const spotterInput = createSpotterInputFromCoordinates(
    spotterPos,
    originalTarget
  );
