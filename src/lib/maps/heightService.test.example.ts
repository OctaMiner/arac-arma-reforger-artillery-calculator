...
async function example4_interpolation() {
  ...
  const heightInterp = getTerrainHeightInterpolated(mapId, coord.east, coord.north)
...
async function example8_artilleryElevation() {
  ...
  const mortarHeight = getTerrainHeightInterpolated(mapId, mortarPos.east, mortarPos.north)

  // Target position
  const targetPos = { east: 7500, north: 5800 }
  const targetHeight = getTerrainHeightInterpolated(mapId, targetPos.east, targetPos.north)
  ...
  const elevationAngle = Math.atan2(heightDiff, horizontalDistance) * (180 / Math.PI)
