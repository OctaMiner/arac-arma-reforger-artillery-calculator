...
console.log('Azimuth:', azimuth.degrees.toFixed(2), '° /', azimuth.mils.toFixed(0), 'MIL (expected 45° / 800 MIL)')
...
console.log('North:', calculateAzimuth(center, north).degrees.toFixed(1), '° (expected 0)')
console.log('East:', calculateAzimuth(center, east).degrees.toFixed(1), '° (expected 90)')
console.log('South:', calculateAzimuth(center, south).degrees.toFixed(1), '° (expected 180)')
console.log('West:', calculateAzimuth(center, west).degrees.toFixed(1), '° (expected 270)')
console.log()
...
console.log('\nOptimal ring for 1500m US HE:', findOptimalRingCount(1500, 'US', 'HE'))
console.log('Optimal ring for 2500m US HE:', findOptimalRingCount(2500, 'US', 'HE'))
console.log('Optimal ring for 500m RUS HE:', findOptimalRingCount(500, 'RUS', 'HE'))
console.log()
...
console.log('  Azimuth:', solution.azimuthDeg.toFixed(2), '° /', solution.azimuthMil, 'MIL')
...
for (const scenario of scenarios) {
  ...
  console.log(`${scenario.name}: Ring ${autoSolution.ringCount} (recommended: ${autoSolution.recommendedCharge})`)
}
console.log()
...
for (const scenario of scenarios2) {
  const testTarget: Coordinate = { east: 600, north: 500, height: scenario.height }
  ...
  console.log(`${scenario.name}: ELEV ${sol.elevationBase} MIL → ${sol.elevationAdj} MIL (Δ${sol.deltaElev} MIL)`)
}
console.log()
