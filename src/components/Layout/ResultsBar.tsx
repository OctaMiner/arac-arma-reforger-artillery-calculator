...
export function ResultsBar() {
  ...
  const chargeStatus = useMemo(() => {
    ...
    const isOptimal =
      fireSolution.recommendedCharge === undefined ||
      fireSolution.ringCount === fireSolution.recommendedCharge;

    return {
      charge: actualCharge,
      isValid:
        actualCharge !== undefined && actualCharge >= 0 && actualCharge <= 4,
      isOptimal,
    ...
    <div className="w-full border-t-2 border-border bg-bg-secondary backdrop-blur-sm">
        ...
        <div className="px-4 py-1 bg-accent-red/10 border-b border-accent-red/30">
          <div className="flex items-center justify-center gap-3 text-xs font-mono text-accent-red">
            ...
            <span className="font-bold">
              {fireSolution.distance > rangeInfo.maxRange
                ? t('results.targetTooFar')
                : t('results.targetTooClose')}
            </span>
            <span className="opacity-80">
              {t('results.tgt')}: {fireSolution.distance}m |{' '}
              {fireSolution.distance > rangeInfo.maxRange
                ? `${t('results.max')}: ${rangeInfo.maxRange}`
                : `${t('results.min')}: ${rangeInfo.minRange}`}
              m
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 gap-4">
        ...
        <div className="flex items-center gap-2 min-w-[100px]">
          {getStatusIcon()}
          <span
            className={`font-mono text-xs font-bold uppercase ${getStatusColor()}`}
          >
            {getStatusText()}
          </span>
        </div>
        ...
        <div className="flex items-center gap-6 flex-1 justify-center">
          ...
          <div className="flex items-center gap-2">
            <Circle
              className={`w-4 h-4 ${chargeStatus?.isValid && status === 'ready' ? 'text-accent-blue' : 'text-muted-foreground'}`}
            />
            <div className="flex items-baseline gap-1">
              <span className="text-xs text-text-secondary uppercase">RNG</span>
              <span
                className={`font-mono text-lg font-bold ${
                  chargeStatus?.isValid && status === 'ready'
                    ? 'text-accent-blue'
          ...
          <div className="flex items-center gap-2">
            <Compass
              className={`w-4 h-4 ${status === 'ready' ? 'text-accent-blue' : 'text-muted-foreground'}`}
            />
            <div className="flex items-baseline gap-1">
              ...
              {windData && fireSolution?.windCorrection && (
                <span
                  className={`text-xs font-mono ml-1 ${
                    fireSolution.windCorrection.azimuthCorrection > 0
                      ? 'text-accent-yellow'
                      : 'text-accent-green'
                  }`}
                >
                  (
                  {fireSolution.windCorrection.azimuthCorrection > 0 ? '+' : ''}
                  {fireSolution.windCorrection.azimuthCorrection.toFixed(1)})
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <ArrowUp
              className={`w-4 h-4 ${status === 'ready' ? 'text-accent-green' : 'text-muted-foreground'}`}
            />
            <div className="flex items-baseline gap-1">
              <span className="text-xs text-text-secondary uppercase">
                ELEV:
          ...
          <div className="flex items-center gap-2">
            <Ruler
              className={`w-4 h-4 ${status === 'ready' ? 'text-text-primary' : 'text-muted-foreground'}`}
            />
            <div className="flex items-baseline gap-1">
              <span
          ...
          <div className="flex items-center gap-2">
            <Clock
              className={`w-4 h-4 ${status === 'ready' ? 'text-accent-yellow' : 'text-muted-foreground'}`}
            />
            <div className="flex items-baseline gap-1">
              <span
                className={`font-mono text-base font-semibold ${
