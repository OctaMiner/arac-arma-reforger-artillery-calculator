...
const CoordinateDisplay = () => {
  const [coords, setCoords] = useState<{ east: number; north: number } | null>(null)
  ...
  if (mortarPosition) {
    ...
    if (distanceToMortar < minRange) {
      ...
      currentRing = findOptimalRingCount(distanceToMortar, mortarConfig.type, mortarConfig.ammo)
  ...
  return (
    ...
    >
      ...
      <div style={{ color: '#9ca3af', fontSize: '12px' }}>
        <span style={{ color: '#93c5fd' }}>E:</span> {meterE.toString().padStart(5, ' ')}m
        <span style={{ margin: '0 8px' }}>|</span>
        <span style={{ color: '#fca5a5' }}>N:</span> {meterN.toString().padStart(5, ' ')}m
      </div>
